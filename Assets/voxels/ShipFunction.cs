...
*/
	...
	void OnGUI(){
		GUI.Box (new Rect (10, 10, 150, 150), "Ship Functions");
		if(GUI.Button(new Rect(20,40,130,20), "Hack Speed")) {
			hackSpeed(target);
		}
		
		// Make the second button.
		if(GUI.Button(new Rect(20,70,130,20), "Suppress Oxygen")) {
			suppressOxygen(target);
		}
		if(GUI.Button(new Rect(20,100,130,20), "Reformat Ship")) {
			activateReformat();
		}
		
		// Make the second button.
		if(GUI.Button(new Rect(20,130,130,20), "Breach Security")) {
			breachSecurity(target);
		}
	}
