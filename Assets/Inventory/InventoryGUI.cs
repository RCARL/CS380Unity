...
public class InventoryGUI : MonoBehaviour {
	...
	private string[] toolbarStrings = {"Engine", "Communication", "Reactor", "Gun"};
	...
	void Start () {
		...
		inventory.addArtificial (Artificial.engine());
	}
	void Update () {
	...
	void OnGUI () {
		...
		switch (toolbarInt) {
		case 0:
			artificialTemp = Artificial.engine();
			i = 0;
			j = 50;
			...
			break;
		case 1:
			artificialTemp = Artificial.communication();
			i = 0;
			j = 50;
			foreach( KeyValuePair<Resource, int> de in artificialTemp.recipe ) {
				GUI.Label (new Rect (middleX + i, middleY + j, 80, 40), de.Key.type, label);
			...
			break;
		case 2:
			artificialTemp = Artificial.reactor();
			i = 0;
			j = 50;
			foreach( KeyValuePair<Resource, int> de in artificialTemp.recipe ) {
				GUI.Label (new Rect (middleX + i, middleY + j, 80, 40), de.Key.type, label);
